...
export function buildApp() {
  ...
  app.get('/api/sessions', async (_, reply) => {
    const result = await pool.query(`
      SELECT s.id, u.username, s.ip_address, s.login_at, s.logout_at, s.duration_sec
      FROM sessions s
  ...
  app.get('/api/events', async (_, reply) => {
    const result = await pool.query(`
      SELECT e.id, COALESCE(u.username, 'system') AS username,
             e.event_type, e.level, e.ip_address, e.message, e.timestamp
      FROM events e
