    ...
    <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{
              ...
              fontSize: 13, color: 'var(--text)', outline: 'none', width: 200,
            }}
          />
          <select
            value={sortBy}
      ...
      <div style={{ maxHeight: 420, overflowY: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={5} style={{ ...tdStyle, textAlign: 'center', color: 'var(--muted)', padding: '2rem' }}>Ничего не найдено</td></tr>
            ) : filtered.map(s => {
                ...
                >
                  <td style={tdStyle}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      }}>{initials(s.username)}</div>
                      {s.username}
                    </div>
                  </td>
                  <td style={{ ...tdStyle, fontFamily: 'monospace', fontSize: 12 }}>{s.ip_address ?? '—'}</td>
                  <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>{fmtTime(s.login_at)}</td>
                  <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>{s.logout_at ? fmtTime(s.logout_at) : '—'}</td>
                  <td style={tdStyle}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 80, height: 4, borderRadius: 2, background: 'var(--border-md)', flexShrink: 0 }}>
                        <div style={{ width: `${barW}%`, height: '100%', borderRadius: 2, background: 'var(--accent)' }} />
                      </div>
